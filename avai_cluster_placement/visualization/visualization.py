...
def draw_line_chart(req_num_list, fig_title, *algorithm_results):
    ...
    for result in algorithm_results:
        ...
        for req_name in req_name_list:
            req_results = np.array(result[FIG_TITLE_DATA_MAP[fig_title]][req_name])
            # mean values
            algo_mean_values.append(req_results.mean())
            # stand deviation
            z_critical = stats.norm.ppf(q=0.95)
            margin_of_error = z_critical * (req_results.std()/math.sqrt(len(req_results)))
            algo_stddev_values.append(margin_of_error)
        # draw
...
def draw_bar_chart(xtick, fig_title, *algorithm_results):
    ...
    for result in algorithm_results:
        ...
        for req_type in req_type_list:
            req_results = np.array(result[FIG_TITLE_DATA_MAP[fig_title]][req_type])
            # mean values
            algo_mean_values.append(req_results.mean())
            # stand deviation
            ...
            algo_stddev_values.append(margin_of_error)
        # draw
        ax.bar(index + i * bar_width, algo_mean_values, bar_width, yerr=algo_stddev_values,
               alpha=opacity, color=LABEL_COLOR_MAP[result["label"]],
